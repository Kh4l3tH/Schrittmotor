...
class Schrittmotor():
    ...
    def move_abs(self, position, speed=2000, offset=None, inverted=None):
        ...
        log('Motor {0}: \033[92m{1:b}\033[0m'.format(self.id, self.status()))
    ...
    def wait(self):
        log('Waiting for Motor {0}'.format(self.id))
        self.cmd.setMotor(self.id)
        time_current = time()
        position_current = self.cmd.getPosition()
        while not self.cmd.isMotorReady():
            if time() - time_current >= 5:
                log('Motor: {0}, Position: {1:.6f}, Status: \033[96m{2:b}\033[0m'.format(self.id, self.get_position(), self.status()))
                time_current = time()
                if position_current == self.cmd.getPosition():
                    log('\033[91mMotor ist vermutlich stehen geblieben!\033[0m')
                    raise ValueError('Motor ist vermutlich stehen geblieben!')
                position_current = self.cmd.getPosition()
            sleep(0.1)

        log('Checking Position Error:')
        self.position_error()
        log('Continuing')
    ...
    def position_error(self):
        ...
        log('Position   Motor: {0}'.format(pos_control))
        log('Position Encoder: {0}'.format(pos_encoder))
        if abs(diff) < 3:
            log('Position Diff: \033[92m{0}\033[0m'.format(diff))
        else:
            log('Position Diff: \033[91m{0}\033[0m'.format(diff))
            log('\033[91mPositionsfehler!\033[0m')
